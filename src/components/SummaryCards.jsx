  ...
  return (
    <div className="summary-grid">
      {fuels.map((fuel) => {
        ...
        const gap =
          cheapest && Number.isFinite(own)
            ? own - cheapest.prices[fuel]
            : null;

        const cheaperCount = Number.isFinite(own)
          ? competitors.filter(
              (c) =>
                Number.isFinite(c.prices?.[fuel]) &&
                c.prices[fuel] < own
            ).length
          : 0;
        ...
        return (
          <article className="summary-card" key={fuel}>
            ...
            <div className="summary-own">
              {Number.isFinite(own) ? `${own.toFixed(1)}p` : '—'}
            </div>
            ...
            <div className="summary-detail">
              ...
              <strong>
                {cheapest ? `${cheapest.prices[fuel].toFixed(1)}p` : '—'}
              </strong>
            </div>

            {cheapest && (
              <div className="muted small">
                {cheapest.name} • {cheapest.distanceMiles.toFixed(1)} mi
              </div>
            )}
