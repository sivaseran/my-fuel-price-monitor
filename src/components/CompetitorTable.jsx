...
export default function CompetitorTable({ competitors, fuels, priorityIds, togglePriority, sort, setSort }) {
  const rows = [...competitors].sort((a, b) => {
    ...
    if (sort === 'unleaded') return (a.prices.unleaded ?? 9999) - (b.prices.unleaded ?? 9999);
    if (sort === 'diesel') return (a.prices.diesel ?? 9999) - (b.prices.diesel ?? 9999);
    ...
    <section className="panel">
      <div className="panel-heading">
        <div>
          ...
          <p>Star as many priority competitors as you want. Priority stations stay pinned at the top.</p>
        </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>Station</th>
              {fuels.map((fuel) => <th key={fuel}>{fuelLabels[fuel]}</th>)}
              <th>Distance</th>
              <th>Updated</th>
            </tr>
          </thead>
          <tbody>
                ...
                <tr key={station.id} className={priority ? 'priority-row' : ''}>
                  <td>
                    <button
                      ...
                      title={priority ? 'Remove priority competitor' : 'Mark priority competitor'}
                    >
                      {priority ? '★' : '☆'}
                    </button>
                  </td>
                  <td>
                    ...
                    <div className="muted">{station.brand || '—'} {priority && <span className="priority-badge">Priority</span>}</div>
                  </td>
                  {fuels.map((fuel) => (
                    <td key={fuel} className="price-cell">
                      {Number.isFinite(station.prices?.[fuel]) ? `${station.prices[fuel].toFixed(1)}p` : '—'}
                    </td>
                  ))}
            ...
            {!rows.length && <tr><td colSpan={fuels.length + 4}>No competitors found in this radius.</td></tr>}
          </tbody>
        </table>
      </div>
    </section>
